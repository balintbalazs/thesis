...
\begin{figure}
    ...
    \bcaption[Excitation and emittion spectrum of enchanced green fluorescent protein (EGFP)]{Excitation spectrum in blue, emission spectrum in yellow. The separeation between the two spectra is due to the Stokes shift.}
    \label{fig:spectrum}
\end{figure}
...
By imaging fluorescently labeled specimens, a wide-field fluorescence microscope hat the capability of discriminating illumination light from fluorescent light due to the Stoles shift described in the previous section. The microscope's operating principle is depicted in Figure \ref{fig:wide-field}.
...
Finally, the light is focused again by a tube lens to create a magnified image in the camera sensor. This type of imaging is called infinity corrected optics, since the back focal length of the objective is in "infinity", meaning that the light exiting the back aperture is parallel. Infinity corrected optics has the advantage that it allows placing various additional optical elements in the infinity space (\textit{i.e.} the space between the objective and the tube lens) without affecting the image quality. In this example such elements are the dichroic mirror and the emission filter. 
...
\begin{figure}
    ...
    \bcaption[Airy pattern]{Airy pattern calculated in Matlab based on Equation (\ref{eq:airy})}
\end{figure}
...
scalar theory of diffraction (no polarization), intensity of PSF Fraunhofer diffraction \cite{born_principles_2013} \cite{sheppard_imaging_1987}
...
\begin{equation}
    v = \frac{2\pi n \Delta r}{\lambda_0} \sin \alpha, \quad
    u=\frac{8\pi n \Delta z}{\lambda_0} \sin^2 \frac{\alpha}{2}
    \label{eq:substitutions}
\end{equation}
and $r = \sqrt{x^2 + y^2}$
$\alpha$ as in Equation (\ref{eq:NA}).

\begin{equation}
    H(0,v) = C_0 \left| \int_0^1 J_0(vr)rdr \right|^2 = \left(2\frac{J_1(v)}{v} \right) ^2,
    \label{eq:airy}
\end{equation}
$J_1$ Bessel function of the first kind, first order, $J_0$ zero order
this is Airy pattern
first minimum at $v=3.83$
Substituting to Equation (\ref{eq:substitutions})

\begin{equation}
    \delta_{xy} = \frac{3.83}{2\pi} \frac{\lambda_0}{n\cdot \sin \alpha} \approx 0.61 \frac{\lambda_0}{NA}
\end{equation}
This is the famous formula derived by .
...
\end{equation} 
first minimum at $u=4\pi$:
\begin{equation}
    \delta_z = \frac{2n\lambda_0}{NA^2}
\end{equation}

final image is psf convolved with fluorophore distribution:
\begin{equation}
    I(u,v) = O(u,v) * H(u,v)
\end{equation}

but this only works for low angles, as the Fraunhofer diffraction equation used to derived these results is only an approximation of the Kirchoff diffraction equation. For high NA objectives, a more reliale way to calculate the resolution is the Stelzer-Grill-Heisenberg principle:
...
\section{3D microscopy techniques}
...
\section{Imaging mouse preimplantation development}

\section{Imaging mouse postimplantation development}

\section{Imaging adult mice}
