...
\subsubsection{High NA imaging, and achieving isotropic resolution}
In the previous paragraphs we defined optical resolution, and derived two formulas to calculate the lateral and axial resolutions. These formulas, however 

but this only works for low angles, as the Fraunhofer diffraction equation used to derived these results is only an approximation of the Kirchoff diffraction equation. For high NA objectives, a more reliable way to calculate the resolution is the Stelzer-Grill-Heisenberg principle:
